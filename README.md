...
> 一个基于 Python 和 Flask-SocketIO 的局域网群聊 + 点对点聊天与文件传输工具。

## 项目简介

**ChatRoom** 是一个专为同一局域网（同一网段）设计的轻量级聊天应用程序。它能够自动扫描并发现网络中的其他在线用户，支持**群聊广播**（消息扇出给局域网内所有在线设备，无需中心服务器）与**点对点私聊**、文件共享。
...
1. **局域网群聊 (Group Chat)**
   - 左侧栏顶部固定「群聊」入口，群聊置顶于私聊列表之上。
   - 发群消息时广播给局域网内所有在线设备，所有人都能看到同一个群。
   - 群聊成员名单与消息历史在本地持久化。
2. **自动网络扫描与发现 (Network Scanning)**
   - 自动检测本机的网络接口与 IP 地址。
   - 自动发现同一网段内正在运行该程序的其他用户。
3. **实时点对点通信 (P2P Messaging)**
   - 采用 HTTP/WebSocket 进行数据传输，消息直接在两个客户端之间传递，保障局域网内的隐私与极低延迟。
4. **Web 浏览器界面 (Web Interface)**
   - 采用现代化、响应式的 Web UI，直接在浏览器中打开即可使用，无需安装额外的桌面客户端。
5. **本地存储与文件管理 (Local Storage)**
   - 所有聊天记录和传输的文件均加密或分类保存在本地。
   - 支持在 Web 界面中自定义存储路径。
6. **高效存储机制 (Efficient Storage)**
...
聊天历史与文件默认保存在以下系统路径中：
- **Windows**: `C:\Users\<用户名>\Documents\ChatRoomData\<用户唯一ID>`
- **macOS/Linux**: `/Users/<用户名>/Documents/ChatRoomData/<用户唯一ID>`

> **提示**：可在 Web 界面的设置面板更改存储路径；自定义路径下会追加 `ChatRoom/<用户唯一ID>` 子目录。

## 📅 未来规划 (Roadmap)

- [ ] **用户昵称与头像**：目前基于 IP 识别，计划加入自定义昵称广播功能。
- [x] **群聊功能**：局域网广播群聊已实现（左侧栏顶部「群聊」入口）。
- [ ] **群文件共享**：在群聊中收发文件。
