...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'add') {
    ...
    if (empty($name) || empty($category)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            $uploaded = handleImageUpload($_FILES['image'], '../../images/transport/');
            if ($uploaded) {
        ...
        if ($messageType !== 'error') {
            $stmt = $db->prepare("INSERT INTO transports (name, category, description, image, detail_content, sort_order) VALUES (:name, :category, :description, :image, :detail_content, :sort_order)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'edit') {
    ...
    if ($id <= 0 || empty($name) || empty($category)) {
        ...
        $sql = "UPDATE transports SET name = :name, category = :category, description = :description, detail_content = :detail_content, sort_order = :sort_order $imageClause WHERE id = :id";
        $stmt = $db->prepare($sql);
        $stmt->execute($params);
        $message = '交通信息更新成功';
        $messageType = 'success';
...
if ($action === 'edit' && isset($_GET['id'])) {
    $stmt = $db->prepare("SELECT * FROM transports WHERE id = :id");
...
if ($action === 'list') {
    ...
    $stmt = $db->prepare("SELECT * FROM transports $whereClause ORDER BY sort_order ASC, id DESC");
    $stmt->execute($params);
    $items = $stmt->fetchAll();
}

$categories = ['到达交通', '市内交通', '周边交通'];
?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tag { display: inline-block; padding: 2px 10px; font-size: 12px; border-radius: 20px; font-weight: 500; background: rgba(59,130,246,0.1); color: #3B82F6; }
...
<body>
    <header class="admin-header">
        ...
        <nav class="admin-header__nav">
            ...
            <a href="manage_transports.php" class="active">交通管理</a>
    ...
    <div class="container">
        ...
        <div class="page-header">
            <h1 class="page-title">交通管理</h1>
            <a href="?action=add" class="btn btn--primary">+ 添加交通</a>
        </div>
        <form method="GET" class="search-bar">
            ...
            <?php if (!empty($searchKeyword) || !empty($searchCategory)): ?>
                <a href="manage_transports.php" class="btn--reset">清除筛选</a>
            <?php endif; ?>
        </form>
        <?php if (!empty($searchKeyword) || !empty($searchCategory)): ?>
        ...
        <div class="page-header"><h1 class="page-title">添加交通信息</h1><a href="manage_transports.php" class="btn btn--outline">← 返回列表</a></div>
        <div class="form-card">
            <form method="POST" action="?action=add" enctype="multipart/form-data">
                <div class="form-group"><label class="form-label">名称 *</label><input type="text" class="form-input" name="name" required placeholder="例如：宁波栎社国际机场"></div>
        ...
        <div class="page-header"><h1 class="page-title">编辑交通信息</h1><a href="manage_transports.php" class="btn btn--outline">← 返回列表</a></div>
        <div class="form-card">
