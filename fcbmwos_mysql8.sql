...
-- ----------------------------
BEGIN;
INSERT INTO `fcbmwo_date` (`id`, `user_id`, `user`, `work_order_no`, `discovered_a_malfunction`, `discovered_a_malfunction2`, `test_results`, `test_results2`, `locate_faulty_components`, `locate_faulty_components2`, `repair_results`, `repair_results2`, `optimization_effect`, `optimization_effect2`, `Discovered_a_malfunction3`, `test_results3`, `locate_faulty_components3`, `repair_results3`, `optimization_effect3`, `created_at`, `updated_at`) VALUES 
...
AFTER INSERT ON `fcbmwo_date`
FOR EACH ROW
BEGIN
    UPDATE fcbmwo_date 
    SET work_order_no = CONCAT('FCBMWO-', DATE_FORMAT(NEW.created_at, '%Y%m%d'), '-', LPAD(NEW.id, 4, '0'))
    WHERE id = NEW.id;
END //
DELIMITER ;

-- 创建触发器：自动生成数据恢复维修工单编号
DELIMITER //
CREATE TRIGGER `tr_drarwo_work_order_no` 
AFTER INSERT ON `drarwo`
FOR EACH ROW
BEGIN
    UPDATE drarwo 
    SET work_order_no = CONCAT('DRARWO-', DATE_FORMAT(NEW.created_at, '%Y%m%d'), '-', LPAD(NEW.id, 4, '0'))
    WHERE id = NEW.id;
END //
DELIMITER ;
...
BEGIN
    IF (NEW.last_login IS NOT NULL) AND 
       (OLD.last_login IS NULL OR NEW.last_login != OLD.last_login) THEN
        INSERT INTO system_log (user_id, username, action, module, description)
        VALUES (NEW.id, NEW.username, 'LOGIN', 'USER', CONCAT('用户 ', NEW.real_name, ' 登录系统'));
    END IF;
END //
DELIMITER ;
...
-- 完成提示
SELECT '飞控板维修工单系统数据库创建完成！' as message;
