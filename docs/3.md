...
本系统是前后端分离的结构，整体上分成两个部分：一部分是专门给游客看的前端展示页面，另一部分是专门给管理员用的后台操作页面。
...
1. **游客页面的设计**：根据大家出门旅游前查资料的习惯，梳理了从首页看概览，到搜索具体景点、查看攻略，最后在留言板互动的全过程。
2. **数据库的设计**：为了存下这些内容，建了7张核心的数据表，把景点、美食、留言等数据关系都理清楚了。
3. **界面的设计**：定下了有宁波特色的配色（青铜金和宁波绿），并考虑到了电脑和手机等不同设备上怎么排版才好看。
