...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const familyNamesSection = document.getElementById('familyNamesSection');
    ...
    function handleFamilyCountSelection() {
        ...
        if (selectedFamilyCount) {
            ...
            if (familyNamesSection) {
                if (familyCount > 0) {
                    familyNamesSection.style.display = 'block';
                } else {
    ...
    // 更新付款区域金额显示（已删除金额显示元素，保留函数以避免错误）
    function updateTotalAmountDisplay(amount) {
        // 金额显示元素已删除，此函数保留以避免调用错误
        return;
    }
            ...
            if (methodValue === 'other') {
                ...
                if (qrCodePlaceholder && qrCodeImage) {
                    if (methodValue === 'wechat') {
                        ...
                        qrCodeImage.innerHTML = `
                            <img src="../../images/wx.jpg" alt="微信收款码" class="img-fluid">
                            <div class="mt-3">
                                <div class="alert alert-warning py-3 mb-2" style="background-color: #fff3cd; border: 2px solid #ffc107;">
                                    <div class="text-center">
                                        <i class="fas fa-exclamation-triangle me-2" style="color: #856404;"></i>
                                        <strong style="color: #856404; font-size: 16px;">
                                            请扫码支付上述金额，支付完成后上传付款凭证
                                        </strong>
                                    </div>
                                </div>
                            </div>
                        ...
                        qrCodePlaceholder.style.display = 'none';
                        qrCodeImage.innerHTML = `
                            <img src="../../images/zbf.jpg" alt="支付宝收款码" class="img-fluid">
                            <div class="mt-3">
                                <div class="alert alert-warning py-3 mb-2" style="background-color: #fff3cd; border: 2px solid #ffc107;">
                                    <div class="text-center">
                                        <i class="fas fa-exclamation-triangle me-2" style="color: #856404;"></i>
        ...
        reader.onload = function(e) {
            uploadPreview.innerHTML = `
                <div class="preview-container text-center">
                    <img src="${e.target.result}" alt="付款凭证" class="preview-image" style="max-width: 90%; max-height: 300px; width: auto; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                    <div class="mt-2">
    ...
    // 获取表单数据
     function getFormData() {
         ...
         // 缴费金额（使用用户输入的金额）
         formData.append('payment_amount', paymentAmountInput ? paymentAmountInput.value : '0');
