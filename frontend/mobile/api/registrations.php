...
class RegistrationsAPI {
    ...
    private function validateRegistrationData($data) {
        $required = ['name', 'phone', 'family_count', 'payment_method', 'payment_amount'];
        ...
        foreach ($required as $field) {
            ...
            // 对于family_count和payment_amount字段，允许值为"0"
            if ($field === 'family_count' || $field === 'payment_amount') {
        ...
        if ($paymentAmount <= 0) {
            return ['valid' => false, 'message' => '请输入有效的缴费金额'];
        }
    ...
    private function createRegistration() {
        try {
            ...
            if ($registrationId) {
                ...
                if ($paymentScreenshotPath) {
                    $response['payment_screenshot'] = $paymentScreenshotPath;
                }
